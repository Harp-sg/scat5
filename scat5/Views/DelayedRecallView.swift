    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            ZStack {
                ...
                case .countdown:
                    DelayedRecallCountdownView(timeRemaining: timeRemaining)
...
private struct RecallingInstructionsView: View, TestController {
    let onStart: () -> Void
    @Environment(SpeechControlCoordinator.self) private var speechCoordinator
...
private struct FinishedView: View, TestController {
    let score: Int
    let wordCount: Int
    let onContinue: () -> Void
    @Environment(SpeechControlCoordinator.self) private var speechCoordinator
    ...
    func executeCommand(_ command: VoiceCommand) {
        switch command {
        case .completeTest, .selectItem, .startTest, .nextTrial:
            onContinue()
        default: break
        }
    }
...
private struct DelayedRecallCountdownView: View {
...
private struct DelayedRecallTest: View, TestController {
