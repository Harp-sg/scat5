...
struct DigitSpanView: View, TestController {
    @Bindable var concentrationResult: ConcentrationResult
    let onComplete: () -> Void
    ...
    func executeCommand(_ command: VoiceCommand) {
        switch command {
        case .submitAnswer, .nextTrial, .startTest:
            processDigitResponse()
        case .completeTest:
            onComplete()
        default: break
...
struct MonthsReverseView: View, TestController {
    @Bindable var concentrationResult: ConcentrationResult
    let onComplete: () -> Void
    ...
    func executeCommand(_ command: VoiceCommand) {
        ...
        case .markIncorrect:
            isCorrect = false
            concentrationResult.monthsCorrect = false
